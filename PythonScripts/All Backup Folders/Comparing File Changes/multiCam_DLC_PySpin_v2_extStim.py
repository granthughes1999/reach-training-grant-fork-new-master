                            ...
                            if record:
                                if start_time == 0:
                                    ...
                                    if not (method == 'roi' and isstim):
                                        ...
                                        stim_val = np.mean(np.sum(frame, axis=0)[:5])
                                        if stim_val > stimThresh:  # GRANT HUGHES --> CHANGE THIS VALUE TO SET THE STIMULUS THRESHOLD
                                            print('/n')
                                                ...
                                                if self.stim_always_armed.value == 1 and not ser_always == 0:
                                                    ...
                                                    if self.events_path:
                                                        with open(self.events_path, "a") as events_file:
                                                            events_file.write("reach_init\t%s\n\r" % self.frm.value)
                                                            events_file.write("reach_roi\t%s\n\r" % self.frm.value)
