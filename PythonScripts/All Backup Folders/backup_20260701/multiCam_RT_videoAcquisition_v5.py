...
# so this new def autoCapture() function is a test for that
    ...
    def _recording_skips_camera(self, camID):
        return (
            getattr(self, "current_mode_tag", None) == "REC"
            and not self.auto_stim.GetValue()
            and self.stim_cam_serial is not None
            and str(camID) == str(self.stim_cam_serial)
        )

    def _abort_record_start(self, message):
        print(f"[ERROR] Record start aborted: {message}")
        try:
            dlg = wx.MessageDialog(
                parent=self,
                message=message,
                caption="Record start aborted",
    ...
    def recordCam(self, event):
        if self.rec.GetValue():
           ...
           # Grant 07-08 ─── Record start time & total duration ───
            ...
            if self.com.value < 0:
                return self._abort_record_start(
                    "Arduino is not available, so the required events file cannot be created. "
                    "Please reinitialize the Arduino/cameras before recording."
                )

            name_base = '%s_%s_%s' % (date_string, self.system_cfg['unitRef'], sess_string)
            path_base = os.path.join(self.sess_dir, name_base)
            events_path = Path(f"{path_base}_events.txt")
            self.events_path = events_path

            try:
                self.ardq.put('recordPrep')
                self.ardq.put(path_base)
                self.ardq_p2read.get(timeout=5.0)
            except Exception as e:
                return self._abort_record_start(
                    f"Arduino did not acknowledge Record prep within 5 seconds: {e}"
                )

            if not events_path.exists():
                return self._abort_record_start(
                    f"Arduino Record prep completed, but the required events file was not created:\n{events_path}"
                )
            ...
            for ndx, s in enumerate(self.camStrList):  # New Code
                ...
                if self._recording_skips_camera(camID):
                    print(f"[INFO] Skipping Record prep for StimCam {camID} (Record mode, stim OFF)")
                    continue
            ...
            if self.com.value > 0:  # New Code
                ...
                self.com.value = 0  # New Code: clear the pending command without marking Arduino unavailable
            ...
            try:  # New Code
                ...
                try:
                    ...
                    print(f"[WARN] recordCam stop: Arduino did not ACK Stop within 5.0s: {e}")
